...
class GBDT_tuning:
	...
	def __init__(self, X, y, cv=5, n_jobs=30, random_state=2018, model_type='classification', scoring=['accuracy', 'roc_auc', 'f1']):
		self.X = X
		self.y = y
		self.cv = cv
		self.n_jobs = n_jobs
	...
	def step2(self, search_type, params):
		...
		params['min_samples_leaf'] = (len(self.y)/np.arange(30, 71, 10)).astype(int)
